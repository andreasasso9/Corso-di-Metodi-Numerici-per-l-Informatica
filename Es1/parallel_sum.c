...
int main (int argc, char **argv)
{
	...
	for(i=0;i<passi;i++)
	{
		...
		if(r==potenze[i]) // Il processore invia
		{
			int sendTo=rank-potenze[i];
			tag=sendTo;
			MPI_Send(&sommaloc,1,MPI_DOUBLE,sendTo,tag,MPI_COMM_WORLD);
		}
		else if(r==0) // il processore riceve
		{
			int recvBy=rank+potenze[i];
			tag=rank;
	...
	if(rank==0)
	{
		...
		FILE *f = fopen("result_par_sum.txt", "a");
        if (f == NULL) {
            perror("Error file could not be opened");
            MPI_Abort(MPI_COMM_WORLD, 1);
        }
        fprintf(f, "Procs:%d; Time:%f; Input:%d\n",
                world_size, T_max, n);
        fclose(f);
	}
